...
主文件：`0910_第一问_药材预热模型_v02_Codex.tex`。正文仅覆盖药材烘干问题的第一问，采用二维轴对称模型，以中截面 z=0 输出径向结果。
...
```text
xelatex -interaction=nonstopmode -halt-on-error 0910_第一问_药材预热模型_v02_Codex.tex
xelatex -interaction=nonstopmode -halt-on-error 0910_第一问_药材预热模型_v02_Codex.tex
```
