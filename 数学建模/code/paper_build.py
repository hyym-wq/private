...
def build():
    ...
    h3(doc, "5.1.2 模型求解")
    body(doc, "方程(1)(2)为含 1/r 奇异项的抛物型偏微分方程，难以求得解析解，采用有限差分法离散求解。其基本思想是：先将连续求解区域用离散网格代替，再把节点上的偏导数用相邻节点函数值的差商近似，从而把偏微分方程化为可求解的代数方程组。对 r∈[0,R]、t∈[0,t_end] 作网格剖分，记空间节点 r_j = jΔr（j=0,1,⋯,N，Δr=R/N）、时间层 t^n = nΔt，节点函数值 u_j^n = u(r_j,t^n)，见图 1。")
    fig(doc, "fig20_时空网格剖分.pdf", "图 1  时空网格剖分（径向 r × 时间 t，节点 u_j^n）")
    body(doc, "导数用差商近似的基本模板为：时间方向用一阶向前差商，空间方向用二阶中心差商，即")
    eq(doc, "∂u/∂t ≈ (u_{j}^{n+1} - u_{j}^{n})/Δt,   ∂²u/∂r² ≈ (u_{j-1}^{n} - 2u_{j}^{n} + u_{j+1}^{n})/Δr²")
    body(doc, "见图 2。")
    fig(doc, "fig21_差商模板.pdf", "图 2  差商模板：（a）一阶向前差商（时间）、（b）二阶中心差商（空间）、（c）一阶中心差商（空间）")
    body(doc, "为保证离散守恒并自然处理 r=0 处的 1/r 奇异项，本文不直接对微分方程逐项做差商，而是采用守恒型有限体积形式：对控制体 [r_{j−1/2}, r_{j+1/2}] 积分式(1)(2)，得半离散格式：")
    ...
    h3(doc, "5.1.3 结果与分析")
    body(doc, "预热平衡阶段（前 30 min）的温度与水分浓度分布见表 1、表 2，剖面演化见图 3。")
    ...
    fig(doc, "fig1_预热平衡阶段剖面.pdf", "图 3  预热平衡阶段药材温度与水分浓度剖面")
    ...
    body(doc, "（1）解析解对比：在常环境温度、纯导热情形下，数值解与分离变量级数解析解的最大误差在 1×10⁻⁴ 量级，验证了 Robin 边界与中心奇点处理的正确性（图 4）。")
    body(doc, "（2）收敛阶：固定时间步，空间方向以 N = 20、40、80、160 加密，温度与水分浓度的无穷范数误差均呈二阶收敛（阶约 2.0）；固定空间步，时间方向以 Δt 加倍，误差亦呈二阶收敛，符合 Crank–Nicolson 格式的理论阶（图 5）。")
    body(doc, "（3）守恒性：全局能量与质量平衡的相对偏差分别为 2.9×10⁻¹⁴ 与 1.9×10⁻¹⁴，在机器精度水平上精确守恒。")
    body(doc, "（4）稳定性：Crank–Nicolson 格式在 Δt 放大到 300 s 时仍与 Δt = 1 s 的结果一致（有界收敛），而显式格式在傅里叶数超过 0.5 后立即非物理放大（图 6），印证了隐式格式的无条件稳定性。")
    fig(doc, "fig4_解析解验证.pdf", "图 4  数值解与级数解析解对比（常环境纯导热）")
    fig(doc, "fig5_收敛阶验证.pdf", "图 5  空间与时间方向的收敛阶验证")
    fig(doc, "fig6_离散格式对比.pdf", "图 6  Crank–Nicolson 与显式格式的稳定性对比")
    h3(doc, "5.1.5 小结")
    ...
    h3(doc, "5.2.2 模型求解")
    body(doc, "在问题一离散格式的基础上，将物性系数在每个时间步冻结于当前状态并对非线性项做牛顿型线性化（扩散系数在状态点处泰勒展开一阶项，含 ∂D/∂C 与 ∂D/∂T 敏度），温度与水分方程弱耦合交替求解，每步 Picard 迭代 2~3 次收敛，三对角方程组用托马斯算法求解。图 7 给出了附录 3 变参数物性随水分浓度的变化。")
    fig(doc, "fig9_变参数物性.pdf", "图 7  附录 3 变参数物性随水分浓度的变化")
    h3(doc, "5.2.3 结果与分析")
    body(doc, "3 h 内的温度与水分浓度分布见表 3、表 4，整个烘干过程的剖面演化见图 8。")
    ...
    fig(doc, "fig10_全烘干过程剖面.pdf", "图 8  整个烘干过程的温度与水分浓度剖面")
    ...
    body(doc, "（2）自适应时间步长。采用步长加倍法做误差控制：以 60 s 为基准宏观步长，逐步长内用变子步长积分并估计误差，误差超限则减半、偏低则加倍。预热期瞬变段自动细分至约 2 s 子步，恒温干燥段放大到 300 s，兼顾精度与效率（图 10）。")
    body(doc, "（3）终止时刻二分定位。以 60 s 为输出节拍判断判据，在跨越判据的相邻时刻间二分，精确定位 max_{r} C = 0.15 的终止时刻。")
    h3(doc, "5.3.3 结果与分析")
    body(doc, "烘干时长 t_{dry} = 205771.8 s = 57.16 h（按分钟向上取整的保守口径为 57.17 h，即 57 h 10 min）。每隔 6 h 的水分浓度见表 5，剖面与干燥前沿见图 9、图 11。")
    ...
    fig(doc, "fig13_全烘干剖面.pdf", "图 9  烘干全过程的水分浓度剖面与特征位置时程")
    fig(doc, "fig15_干燥前沿.pdf", "图 10  干燥前沿随时间的推进")
    fig(doc, "fig12_自适应步长.pdf", "图 11  自适应时间步长与误差控制")
    body(doc, "由图 10 可知，水分浓度等值线（干燥前沿）由表面向内部逐步推进，中心是最后达标的位置；表 5 中 57.17 h 时中心水分浓度恰降至 0.1500，各处均已低于 0.15，满足烘干要求。")
    h3(doc, "5.3.4 检验分析")
    ...
    h3(doc, "5.4.3 结果与分析")
    body(doc, "考虑收缩后的烘干时长 t_{dry} = 188610 s = 52.39 h（按分钟向上取整的保守口径为 52.40 h）。每隔 6 h 的水分浓度见表 6，半径收缩、移动边界剖面与时程见图 12~14。")
    ...
    fig(doc, "fig16_半径收缩与对流.pdf", "图 12  附件 2 收缩半径 R(t) 及其中心差分数值微分")
    fig(doc, "fig17_移动边界剖面.pdf", "图 13  移动边界下的物理坐标剖面与归一化坐标剖面")
    fig(doc, "fig18_特征位置时程.pdf", "图 14  特征位置水分浓度时程与收缩效应对比")
    body(doc, "由图 12 可知，收缩集中在前约 24 h，此后半径稳定在约 1.198 cm；图 13 中物理坐标剖面的右端随收缩内移，归一化坐标下边界恒在 E = 1、剖面因对流项被整体抬升（浓缩效应）。收缩对烘干时长的影响显著：有收缩时 t_{dry} = 52.4 h，而将半径冻结为 2 cm（无收缩）时 t_{dry} = 129.1 h，缩短约 2.46 倍（对应 R² 缩小 2.79 倍、扩散路径缩短与 1/R² 放大共同作用）。")
    h3(doc, "5.4.4 检验分析")
    body(doc, "（1）收敛性：空间方向 N = 100、200、400、800 时 t_{dry} 依次为 52.37、52.38、52.40、52.40 h，N ≥ 400 已收敛；时间方向 Δt = 120、60、30 s 均给出 52.39~52.40 h，结论与步长无关（图 15）。")
    ...
    fig(doc, "fig19_验证与对比.pdf", "图 15  问题四的空间收敛、对流项符号检验与烘干时长对比")
    h3(doc, "5.4.5 小结")
