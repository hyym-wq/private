...
{\normalsize ——基于节点分类离散与线性化隐式差分的一维热质传递模型}
\end{center}
...
\caption{径向节点三类离散示意图：（a）中心点 $j=0$ 用 L'H\^opital 法则与对称虚拟点；（b）内部点 $1\le j\le N-1$ 用柱坐标守恒型中心差分；（c）边界点 $j=N$ 用 Robin 半控制体}
\label{fig:nodes}
\end{figure}
