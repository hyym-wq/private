...
def fig_param():
    ...
    ax = axes[1]
    for Tlab, col in ((28.0, CAT[0]), (50.0, CAT[1])):
        ax.semilogy(Cc, m.diff_coef(Cc, Tlab), color=col,
                    label=f"$T$={Tlab:.0f} °C")
...
def fig_dry(times, r, T, C):
    ...
    for k, th in enumerate([0.0, 0.5, 1.0, 2.0, 3.0]):
        i = int(round(th * 3600 / 1800))
        axes[0].plot(r_cm, T[i], color=SEQ7[k + 1], lw=2.0, zorder=3)
        axes[0].annotate(f"{th:g} h", xy=(r_cm[-1], T[i][-1]),
    ...
    for k, th in enumerate([0.0, 6.0, 12.0, 24.0, 48.0, 72.0]):
        ...
        axes[1].annotate(f"{th:g} h", xy=(r_cm[-1], C[i][-1]),
                         xytext=(4, 0), textcoords="offset points",
                         color=INK2, fontsize=8, va="center")
    axes[1].axhline(0.15, color=CRIT, lw=1.2, ls=(0, (4, 3)), zorder=2)
    axes[1].text(2.08, 0.15, "烘干标准 $C$=0.15", color=CRIT, fontsize=8,
                 va="bottom", ha="right")
...
# ---------------------------------------------------------------------------
def _one_step_residual(newton, T_old, C_old, Ti_o, Ti_n, Ci_o, Ci_n, dt, dr, N, theta):
...
def fig_conv():
    ...
    resN = _one_step_residual(True, T_old, C_old, Ti_o, Ti_n, Ci_o, Ci_n, dt, dr, N, theta)
    resL = _one_step_residual(False, T_old, C_old, Ti_o, Ti_n, Ci_o, Ci_n, dt, dr, N, theta)
    ...
    ax.axhline(1e-8, color=MUTED, lw=1.0, ls=(0, (4, 3)), zorder=2)
    ax.text(1.02, 1e-8, "容差 $10^{-8}$", color=MUTED, fontsize=8, va="bottom")
    style(ax, "Picard 迭代次数 $k$", "$\Vert C^{(k)}-C^{(k-1)}\Vert_\infty$（对数）",
          "单步迭代收敛对比（预热期 $t=1800$ s）")
